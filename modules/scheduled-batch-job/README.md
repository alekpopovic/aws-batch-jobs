...
- `modules/batch-fargate`
- `modules/stepfunctions-batch-submit`
- `modules/eventbridge-scheduler-sfn`
...
- Private subnet with VPC endpoints: keep `assign_public_ip = false`; pair this wrapper with `modules/vpc-endpoints-fargate` so Fargate can reach ECR, CloudWatch Logs, and S3 privately.
